...
<?php  
    ...
    $productList = executeResult("SELECT * FROM products WHERE title LIKE '%Cáp%' OR title LIKE '%Củ sạc%'");
?>
<!-- body -->
<div class="container">
    <!-- Advertisement Image -->
                    ...
                    if (!empty($nameSearch)){
                        ...
                        foreach ($productList as $item) {
                            echo '<div class="col-md-4 col-6 box-product">
                                    ...
                                    <p class="price">'.number_format($item['price'], 0, ',', ' ') .' '. '<span s="currency">đ</span>' . '</p>
                                  </div>';
                        }
                    }
                ?>
